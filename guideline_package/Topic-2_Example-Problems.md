...
Instead of asking for a specific library (which might lead the LLM to use deprecated methods like `yaml.load`), the developer describes the _requirement_ and the _constraint_.
...
The LLM selects `json` or a cryptographically signed alternative because it was prompted for _functionality_ and _security constraints_ rather than a library name.
